...
for x in $(cat /tmp/bloquear_ip_txt | sort | uniq); do iptables -A INPUT -s $x -p tcp --dport 22 -j DROP ; done
for x in $(cat /tmp/bloquear_ip_txt | sort |uniq); do echo  ALL: $x >> /etc/hosts.deny ; done
